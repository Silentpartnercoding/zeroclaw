...
| Chat and channel sessions | `[channels].session_backend` plus `SessionBackend` | Default `data/sessions/sessions.db`; legacy/explicit JSONL uses `data/sessions/*.jsonl` plus `*.metadata.json` ownership sidecars | `zeroclaw-infra` session backend shared by channels, gateway, RPC tools | SQLite backend uses WAL; JSONL message mutations and metadata replacement share one per-directory lock; `SessionActorQueue` serializes active turns per session | Chat/Code sessions use this unified backend. Legacy JSONL files without a sidecar remain readable but unattributed, so agent-scoped session tools fail closed until a trusted gateway or channel path stamps ownership. Switching to SQLite imports and archives both files. ACP protocol sessions use a separate store. |
