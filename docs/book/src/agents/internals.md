...
The post-persist side effects are best-effort and report surfaced failures, but archive-file write failures may appear only in gateway logs. Delete retries use the shared residue resolver. Committed-rename recovery is narrower: the gateway API can retry residue left under the old alias through a gateway-local probe, while the CLI cannot yet retry after the source alias has been durably removed. After deletion, verify the archive contents and logs before relying on the archive for recovery. Automated restore is not supported.
